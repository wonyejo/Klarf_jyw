...
게임 개발 중 플레이했을 때 같이 게임을 하는 두 플레이어가 보는 화면이 달라지는 것을 보게 되었습니다.
이는 처음에 동기화를 고려하지 않고 간단하게 멀티 플레이 기능을 구현했기 때문에 네트워크 지연과 데이터 손실 등의 이유로 발생한 문제였습니다. 동기화 작업을 위해 락스텝, 서버 시뮬레이션 방식 등 여러 동기화 이론을 공부했습니다.
서버 시뮬레이션 방식은 서버에서 물리 연산이 가능해야 했지만 사용하고 있는 서버의 사양이 낮아 이 게임의 동기화 방식으로 채택할 수 없었습니다. 또한, 두 방식 모두 사용자의 입력을 전송해야 했습니다. 그러나 이미 캐릭터의 절대 좌표를 서버에 전송하고 서버에서는 다른 플레이어에게 이를 브로드캐스팅 하는 방식으로 구현하여 일반적인 락스텝 방식도 불가능했습니다.
많은 고민과 시행착오 끝에 분신 객체라는 개념을 고안해냈습니다. 사용자의 입력을 통해 로컬에서는 렌더링하지 않아 보이지 않는 분신 객체를 먼저 움직이고 서버에서는 이 객체의 좌표를 처리하도록 구현했습니다. 서버와 클라이언트의 부하를 줄이기 위해 위치 정보를 모아뒀다가 일정 시간 간격으로 전송하고 그에 따라 클라이언트 측에서 부드러운 움직임을 위한 보간을 하는 등의 작업을 추가하여 동기화의 퀄리티를 높였습니다. 또한, 물리적 충돌이나 래그돌로 Physic이 전환되는 등의 이벤트에 의해 분신 객체가 캐릭터 객체로 돌아오도록 구현해 분신 방식으로 인해 발생할 수 있는 버그를 해결하였습니다.
...
팀 단위의 프로젝트 진행 시 가장 중요하게 생각하는 부분은 효율적인 협업입니다. 이를 가장 중요하게 생각하는 이유는 다음 두 가지입니다.
첫째로, 제가 졸업 프로젝트를 진행할 때, 협업의 중요성을 몸소 느꼈습니다. 프로젝트에서 역할과 책임이 명확하게 정의되고 작업 일정이 체계적으로 관리되지 않았을 때, 프로젝트가 지연되고 업무 간의 혼선이 발생하는 등의 어려움을 겪었습니다. 이러한 경험을 통해 효율적인 협업이 프로젝트의 성패에 직접적으로 영향을 미치며, 업무의 원활한 진행을 보장한다는 것을 깨달았습니다.

둘째로, 효율적인 협업은 팀 내에서의 원활한 커뮤니케이션과 업무 분담을 가능하게 합니다. 팀원들 간의 역할과 책임이 명확하게 정의되면 작업의 중복을 방지하고 각자가 맡은 부분에 집중할 수 있습니다. 이는 프로젝트의 효율성을 향상시키고 결과물의 품질을 높입니다. 이러한 이유로 효율적인 협업을 통해 프로젝트의 성공 확률을 높일 수 있다는 것을 깨달았습니다.
